...
class BlogPostModel {
    ...
    static async getAll(options = {}) {
        ...
        if (search) {
            whereClause.push('(MATCH(bp.title, bp.excerpt, bp.content) AGAINST(? IN BOOLEAN MODE) OR bp.title LIKE ? OR bp.excerpt LIKE ?)');
    ...
    static async getByCategory(categorySlug, limit = 10) {
        ...
        try {
            const [rows] = await pool.execute(query, [categorySlug, limit]);
            return rows;
        } catch (error) {
            throw error;
        }
